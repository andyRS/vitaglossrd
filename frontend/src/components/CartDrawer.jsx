          ...
          >
            ...
            {items.length > 0 && (
              <div className="border-t border-gray-100 px-5 py-4 space-y-3">
                ...
                {step === 'cart' && (
                  <>
                    {/* ── Pagar en línea (tarjeta o PayPal) ── */}
                    <button
                      onClick={() => { setOpen(false); navigate('/checkout') }}
                      className="w-full bg-primary hover:bg-blue-800 text-white font-bold py-4 rounded-2xl transition-colors flex items-center justify-center gap-2 text-base shadow-lg shadow-blue-200"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                      </svg>
                      Pagar en línea (Tarjeta / PayPal)
                    </button>

                    {/* ── WhatsApp (pago por transferencia o contra entrega) ── */}
                    <button
                      onClick={handlePedirClick}
                      className="w-full bg-[#25D366] hover:bg-[#1ebe5d] text-white font-bold py-3 rounded-2xl transition-colors flex items-center justify-center gap-2 text-sm shadow-lg shadow-green-200"
                    >
